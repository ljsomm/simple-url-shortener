...
jest.mock("../../../../main/adapters/datasources/repositories/url.repository", () => {
	return {
		URLRepository: {
...
describe('URLUseCase', () => {
	...
	it('should create and retrieve shortened URL', async () => {
		expect(await URLUseCase.createNewShortURL("https://lucasjsdev.com")).toEqual({ shortened: `http://localhost:3030/${btoa('1')}` });
	});

	it('should retrieve the original URL', async () => {
		console.log(`http://localhost:3030/${btoa('1')}`);
		expect(await URLUseCase.findOriginalURL(`${btoa('1')}`)).toEqual(`https://lucasjsdev.com`);
	});
})